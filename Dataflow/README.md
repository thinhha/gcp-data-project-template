...
./gradlew run -Dpipeline="gcs2bigquery" -Dorg.gradle.java.home="/usr/lib/jvm/java-8-openjdk-amd64" --args="--runner=DataFlowRunner --project='$PROJECT_ID' --tempLocation='gs://$GCS_BUCKET/dataflow' --subnetwork=https://www.googleapis.com/compute/alpha/projects/$PROJECT_ID/regions/europe-west4/subnetworks/my-vpc-subnet --region=europe-west1 --zone=europe-west4-a --usePublicIps=false --inputGCSPath=gs://$GCS_BUCKET/data/sample.json"
```
